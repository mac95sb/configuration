{ pkgs, inputs, ... }: {
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      # Floating shell hook
      set-hook -g session-created 'if -F "#{==:#{session_name},tdl-shell}" "set-option -t tdl-shell status off \; split-window -h -p 50 -t tdl-shell:1.1 -c \"#{pane_current_path}\" \; select-pane -t tdl-shell:1.1"'
