{ pkgs, lib, ... }:
let
  # Resolve Nix store paths for language servers that need runtime path discovery.
  # These are passed to Lua as vim globals so the config stays pure.
  vueLS =
...
in
{
  programs.nvf = {
    ...
    settings.vim = {
      ...
      # Plugins managed by Nix — replaces vim.pack.add in init.lua
      startPlugins = with pkgs.vimPlugins; [
        blink-cmp
        mini-nvim
        (nvim-treesitter.withPlugins (p: with p; [
          lua python typescript javascript vue
          # swift is installed by Xcode; add grammar if available in your nixpkgs
        ]))
        conform-nvim
      ];

      # LSP binaries and formatters available on neovim's PATH.
      # sourcekit-lsp is provided by Xcode and picked up automatically.
      ...
      luaConfigRC.init = # lua
        ''
          ...
          -- Nix store paths (set before any LSP config reads them)
          ...
          -- ––– Treesitter –––
          -- Parsers are pre-installed by Nix; no runtime install needed.
                  ...
                  local function try()
                    ...
                    if not ts_client then
                      if retries < 10 then
                        ...
                        vim.notify("Could not find ts_ls required by vue-language-server.", vim.log.levels.ERROR)
                      end
                      return
                    end
