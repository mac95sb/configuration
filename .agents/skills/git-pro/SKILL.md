...
- When asked to create a clean commit history from dirty config files, validate first, then stage and commit by logical concern rather than by incidental file order. Fetch before pushing, confirm the local branch is only ahead of upstream, push, and finish with commit subjects plus clean/aligned status. See `references/dotfile-clean-commit-history.md` for a concrete checklist.
